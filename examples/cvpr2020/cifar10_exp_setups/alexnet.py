...
class torch_alexnet:
  def __init__(self):
    ...
    self.name = 'torch_alexnet'
    self.model = alexnet(weights=AlexNet_Weights.IMAGENET1K_V1).to(self.device)
    self.model.loss = torch.nn.CrossEntropyLoss()
    # change to imagenet dataset?
    #self.train_loader, self.test_loader = data_loader(batch_size=128, n_workers=4, dataset="CIFAR10")
    self.train_loader, self.test_loader = get_ImageNet(data_dir='./bucket_images_train_small.txt',batch_size=16,val_batch_size=16)
