...
class resnet_conv_only():
  ...
  def finetune_setup(self, tag_of_lc_model, c_step_config):
    ...
    model_state_to_load = exp_run_details['model_states'] # despite the 's' at the end, there is only one model state, the last
    ...
    for i, module in enumerate(
        [x for x in self.model.modules() if isinstance(x, nn.Conv2d)]):
      module.selected_rank_ = compression_info[f"task_{i}"]['selected_rank']
      if self.parametrization == 'advanced':
        self.params_subtract_adv += module.selected_rank_ ** 2
        print('module.selected_rank_ **2', (module.selected_rank_ ** 2))
        self.flops_subtract_adv += (module.selected_rank_ ** 2) * module.multiplier
      print(module.selected_rank_, self.params_subtract_adv, self.flops_subtract_adv)
    ...
    print('The number of FLOPS in this model', compressed_flops[1] - self.flops_subtract_adv)
    print('The number of params in this model:', compressed_params - self.params_subtract_adv)
    print('ADV subtract values:')
    print('  flops: ', self.flops_subtract_adv)
    print('  params: ', self.params_subtract_adv)
    flops_rho = (all_flops[0]) / (compressed_flops[0] - 2.0 * self.flops_subtract_adv)
    storage_rho = (all_params) / (compressed_params - self.params_subtract_adv)
...
class resnet20_conv_only(resnet_conv_only):
  def __init__(self):
    super(resnet20_conv_only, self).__init__("resnet20_conv_only", resnetcif20(), 'resnetcif20')


class resnet32_conv_only(resnet_conv_only):
  def __init__(self):
    super(resnet32_conv_only, self).__init__("resnet32_conv_only", resnetcif32(), 'resnetcif32')


class resnet56_conv_only(resnet_conv_only):
  def __init__(self):
    super(resnet56_conv_only, self).__init__("resnet56_conv_only", resnetcif56(), 'resnetcif56')


class resnet110_conv_only(resnet_conv_only):
  def __init__(self):
    super(resnet110_conv_only, self).__init__("resnet110_conv_only", resnetcif110(), 'resnetcif110')