from lc.models.torch.vggcif import vggcif16_bn, vggcif19_bn
...
class vgg_all():
  ...
  def finetune_setup(self, tag_of_lc_model, c_step_config):
    ...
    reparametrize_low_rank(self.model, old_weight_decay=old_weight_decay)
    print(self.model)
...
class vgg16_all(vgg_all):
  def __init__(self):
    super(vgg16_all, self).__init__("vgg16_all", vggcif16_bn(), 'vggcif16_bn')


class vgg19_all(vgg_all):
  def __init__(self):
    super(vgg19_all, self).__init__("vgg19_all", vggcif19_bn(), 'vggcif19_bn')
